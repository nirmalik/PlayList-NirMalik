...
try {
    ...
    $type = $_GET['type'];
    if ($type == 'playlist') {
        // playlist songs and search playlist
        if (isset($_GET['id'])) {
    ...
    switch ($_SERVER['REQUEST_METHOD']) {
        ...
        case 'POST':
            ...
            if (empty($update)) {
                ...
                $update['id'] = $id;
                $stmt = $conn->prepare('UPDATE playlists SET ' . implode(", ", array_map(function($v) {
                                    return "{$v}=:{$v}";
                                }, array_keys($update))) . ' WHERE id=:id');
                $stmt->execute($update);
                response(TRUE);
            }
            break;
...
function playlist_search($keyword) {
    global $conn;
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            $data = array();
            $stmt = $conn->prepare("SELECT id,name,image FROM playlists WHERE UPPER(name) LIKE '%$keyword%'");
            if ($stmt->execute()) {
...
function playlist_songs($id) {
    ...
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            ...
            $mega = array($id, json_decode($data['songs']));
            response(TRUE, [
                'songs' => empty($data['songs']) ? [] : json_encode($mega, TRUE)
            ]);
            break;
        case 'POST':
            $okresult = false;
